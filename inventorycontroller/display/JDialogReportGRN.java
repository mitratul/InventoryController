 ...
 * @author  brinto
 */
public class JDialogReportGRN extends javax.swing.JDialog {
    
    /** Creates new form JDialogReportGRN */
    public JDialogReportGRN(inventorycontroller.display.JFrameMDI parent, boolean modal, 
    ...
    private void initComponents(inventorycontroller.display.JFrameMDI parent, 
     inventorycontroller.function.DbInterface dbi) {
    	...
    	grnList=null;
    	selectedGrnList=new java.util.Vector<String>();
    ...
    private void generatePerform(){
    	...
    	for (int i = 0; i<tm.getRowCount(); i++){
    		if((Boolean)tm.getValueAt(i, 0)){
    			selectedGrnList.add(new String(tm.getValueAt(i, 1).toString()));
    		}
    	}
    ...
    private void createReport(){
    	String[] grnNos=new String[selectedGrnList.size()];
    	for (int i = 0; i<grnNos.length; i++){
    		grnNos[i]=selectedGrnList.elementAt(i);
    	}
    	inventorycontroller.function.GrnReportProcessor grp=
    	 new inventorycontroller.function.GrnReportProcessor(dbInterface1, grnNos);
    	grp.generate();
    ...
    private void fillGRNTable(){
    	grnList=new inventorycontroller.function.GoodsReceiveNoteProcessor(dbInterface1)
    	 .getGrnForGrnReport();
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            grnList,
            new String [] {
                "Select", "GRN No.", "GRN Amt.", "GRN Date", "PO No.", "Vendor No.", "Vendor Name"
            }
        ) {
    ...
    private Object[][] grnList;
    private java.util.Vector<String> selectedGrnList;
